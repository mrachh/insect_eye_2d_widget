...
\vspace*{2ex}\newline
You will need to switch to the \textit{dev-rcip} branch to use the widget. This can be done by running
\vspace*{2ex}\newline
\ovalbox{git checkout dev-rcip} 
\vspace*{2ex}\newline
Then you can run \textit{startup.m} to add both chunkie and its dependencies to the
MATLAB path. 

\subsection{Option 2: Package download and install}
Download the \textit{dev-rcip} branch of chunkie from~\url{https://github.com/fastalgorithms/chunkie} 
and download FLAM from~\url{https://github.com/klho/FLAM}. In each of the respective directories
run \textit{startup.m} to add the relevant directories to the MATLAB path.

\subsection{Testing the installation}

In order to test the accurate installation of the correct version of the chunkie package,
you can run \textit{testclm4.m} located at \textit{chunkie/clmtest}. 
...
available in the ``fmm2d\_matlab\_binaries'' folder. To make them available to the widget, 
just run ``startup\_fmm2d.m'' to add the relevant folder to the PATH. 

To verify successful installation of the fmm2d library, you can run the {\textit test\_fmm.m} 
script in the {\textit folder}. It should report back with ``Successfully completed 2 out of 2 tests in fmm2d package''.

In case the installation fails, you can generate the MATLAB interfaces for the fmm2d library by first getting 
the package from~\url{https://github.com/flatironinstitute/fmm2d}. You will need to copy over the appropriate {\textit make.inc.$\star$} file, depending
on your operating system and compiler installed to {\textit make.inc}  
followed by running ``make matlab'' and ``matlab/startup\_fmm.m''.  For example on a windows machine, you would run the following from a command prompt
\vspace*{2ex}\newline
\ovalbox{copy make.inc.windows make.inc} \vspace*{1ex}\newline \ovalbox{make matlab} 
\vspace*{2ex}\newline
You would still need to run the ``matlab/startup\_fmm.m'' script from MATLAB to set the path.
For detailed installation instructions see~\url{https://fmm2d.readthedocs.io/en/latest}. 
If you are going this route, make sure that the path to the \textit{fmm2d/matlab} folder is higher in priority
in your MATLAB path than the \textit{insect\_eye\_2d\_widget/fmm2d\_matlab\_binaries} folder. 
